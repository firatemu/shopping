...
export class SalesService {
    ...
    async checkout(tenantId: string, dto: CreateOrderDto, userId: string) {
        return this.prisma.executeTransaction(async (tx) => {
            ...
            // 2. Resolve cart items and validate stock
            ...
            for (const item of dto.items) {
                ...
                const unitPrice = variant.salePrice ?? variant.product.salePrice;
                const kdvRate = variant.product.kdvRate;
                const lineSubtotal = new Decimal(unitPrice.toString()).mul(item.quantity);
                const kdvAmount = lineSubtotal.mul(kdvRate).div(100);
                const lineTotal = lineSubtotal.add(kdvAmount);
            ...
            const grandTotal = subtotal.add(kdvTotal).sub(discountTotal);
            ...
            const voucherRowsByCode = new Map<
                string,
                { id: string; code: string; currentBalance: Decimal; status: GiftVoucher['status']; expiresAt: Date | null }
            >();
            ...
            await tx.auditLog.create({
                data: {
                    ...
                    entityId: order.id,
                    action: 'CREATE',
                    newValue: { orderNumber, grandTotal: grandTotal.toString(), itemCount: resolvedItems.length } as any,
                },
            });
