...
describe('CashRegisterService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [CashRegisterService, { provide: PrismaService, useValue: prisma }],
    }).compile();
  ...
  describe('openSession', () => {
    it('should open a new session', async () => {
      ...
      const result = await service.openSession(mockTenantId, mockCashierId, {
        openingBalance: 1000,
      });
    ...
    it('should reject if session already open', async () => {
      ...
      await expect(
        service.openSession(mockTenantId, mockCashierId, { openingBalance: 1000 }),
  ...
  describe('createAdjustment', () => {
    it('should allow adjustment on closed session', async () => {
      ...
      prisma.cashRegisterAdjustment.create.mockResolvedValue({
        id: 'adj-1',
    ...
    it('should reject if not CLOSED', async () => {
      prisma.cashRegisterSession.findFirst.mockResolvedValue(null);
      await expect(
        service.createAdjustment(mockTenantId, mockSessionId, 50, 'test', mockCashierId),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
