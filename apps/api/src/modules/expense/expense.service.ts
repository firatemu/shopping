...
export class ExpenseService {
  ...
  private toJsonExpense(row: ExpenseWithCategory & { createdByUser?: { firstName: string; lastName: string } | null; updatedByUser?: { firstName: string; lastName: string } | null }) {
    return {
      id: row.id,
  ...
  async create(
    ...
    this.logger.log(
      `[tenantId=${tenantId}] ${dto.type} created: categoryId=${dto.categoryId} — ${dto.amount}`,
    );
  ...
  async findAll(
    ...
    return {
      data: rows.map((e) => this.toJsonExpense(e as ExpenseWithCategory & { createdByUser?: { firstName: string; lastName: string } | null; updatedByUser?: { firstName: string; lastName: string } | null })),
  ...
  async update(
    ...
    const data: any = { ...dto, updatedBy: _userId };
  ...
  async getSummary(tenantId: string, options: { dateFrom: string; dateTo: string }) {
    ...
    for (const e of expenses) {
      ...
      if (e.type === ExpenseType.INCOME)
        byCategory[key].income = byCategory[key].income.add(e.amount);
