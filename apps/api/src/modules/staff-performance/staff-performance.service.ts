...
export class StaffPerformanceService {
    ...
    async setTarget(tenantId: string, dto: { userId: string; period: string; targetAmount: number; commissionRate?: number }, _managerId: string) {
        const target = await this.prisma.salesTarget.upsert({
            ...
            create: { tenantId, userId: dto.userId, period: dto.period, targetAmount: dto.targetAmount, commissionRate: dto.commissionRate ?? 0 },
            update: { targetAmount: dto.targetAmount, commissionRate: dto.commissionRate ?? undefined },
        });
        this.logger.log(`[tenantId=${tenantId}] Sales target set: user=${dto.userId} period=${dto.period} target=${dto.targetAmount}`);
        return target;
    }
