...
export class ProductBrandService {
  ...
  async remove(tenantId: string, id: string, userId: string) {
    const row = await this.ensureBrand(tenantId, id);
    ...
    const productLinked = await this.prisma.product.count({
      where: {
        ...
        isDeleted: false,
        OR: [
          { brand: { equals: row.name, mode: 'insensitive' } },
          { brand: { equals: row.code, mode: 'insensitive' } },
        ],
      },
    });
    if (productLinked > 0) {
      throw new BadRequestException('Bu markaya bağlı ürünler varken silinemez');
    }
