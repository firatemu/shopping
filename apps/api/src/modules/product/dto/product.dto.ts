...
export class CreateVariantDto {
    @ApiPropertyOptional({ description: 'Manuel barkod (boşsa sistem üretir)', example: 'DEM000001SYH0M07' })
    @IsOptional()
    @IsString()
    @MaxLength(16)
    barcode?: string;
    ...
    @ApiPropertyOptional({ example: 139.90 })
    @IsOptional()
    @IsNumber()
    @Min(0)
    salePrice?: number;
...
export class CreateProductDto {
    ...
    @ApiProperty({ example: 45.00, description: 'Cost price (DECIMAL 12,2)' })
    @IsNumber()
    @Min(0)
    costPrice: number;

    @ApiProperty({ example: 129.90, description: 'Sale price (DECIMAL 12,2)' })
    @IsNumber()
    @Min(0)
    salePrice: number;
