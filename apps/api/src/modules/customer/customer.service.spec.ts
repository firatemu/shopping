...
describe('CustomerService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: PrismaService, useValue: prisma },
        {
          ...
          useValue: { generateExcel: jest.fn(), generateCustomerStatementPdf: jest.fn() },
        },
      ],
    }).compile();
    ...
    it('should create a customer with default credit limit', async () => {
      ...
      expect(prisma.customer.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ tenantId: mockTenantId, name: 'Mehmet' }),
        }),
      );
    });
  });
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException when not found', async () => {
      ...
      await expect(service.findById(mockTenantId, 'nonexistent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('recordPayment', () => {
    it('should deduct payment from customer balance', async () => {
      ...
      prisma.ledgerMovement.create.mockResolvedValue({
        id: 'mov-1',
    ...
    it('should throw if customer not found', async () => {
      ...
      await expect(
        service.recordPayment(
          mockTenantId,
          { customerId: 'bad', method: 'PAYMENT_CASH' as any, amount: 100 },
          mockUserId,
        ),
