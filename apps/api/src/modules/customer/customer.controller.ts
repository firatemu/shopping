...
export class CustomerController {
  ...
  @ApiOperation({ summary: 'Cari ekstre Excel indir' })
  ...
  async exportStatementExcel(
    ...
    const buffer = await this.customerService.exportStatementExcel(tenantId, customerId, {
      dateFrom,
      dateTo,
    });
    const safeCode = customerId.slice(0, 8);
    res.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': `attachment; filename=cari_ekstre_${safeCode}.xlsx`,
    });
    res.send(buffer);
  }

  @Get(':id/statement/export/pdf')
  @ApiOperation({ summary: 'Cari ekstre PDF indir (kurumsal şablon)' })
  @ApiQuery({ name: 'dateFrom', required: false })
  @ApiQuery({ name: 'dateTo', required: false })
  ...
  async exportStatementPdf(
    ...
    const buffer = await this.customerService.exportStatementPdf(tenantId, customerId, {
      dateFrom,
      dateTo,
      paper,
      orientation,
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename=cari_ekstre_${safeCode}.pdf`,
    });
    res.send(buffer);
  }
  ...
  async getStatement(
    ...
    return this.customerService.getStatement(tenantId, customerId, {
      dateFrom,
      dateTo,
      page,
      limit,
