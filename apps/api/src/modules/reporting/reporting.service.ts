...
export class ReportingService {
    ...
    async getMobileDailySummary(tenantId: string, date: string) {
        ...
        const avgBasket =
            totalSales > 0 ? revenueNum.div(totalSales) : new Decimal(0);
        return {
            date,
            kpis,
            todaySales: daily,
    ...
    async getSalesTrend(tenantId: string, days: number) {
        ...
        const orders = await this.prisma.order.findMany({
            where: {
                ...
                status: { in: [OrderStatus.COMPLETED, OrderStatus.PARTIALLY_RETURNED, OrderStatus.FULLY_RETURNED] },
            },
    ...
    async getCashRegisterSessionsReport(tenantId: string, dateFrom: string, dateTo: string) {
        ...
        return {
            ...
            cashFlow: { cashIn: totalIn.toString(), cashOut: totalOut.toString(), net: totalIn.sub(totalOut).toString() },
