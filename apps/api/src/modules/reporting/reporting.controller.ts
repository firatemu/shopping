...
export class ReportingController {
  ...
  async exportDailySalesExcel(
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename=satis_${date}.xlsx`,
    });
    res.send(buffer);
  }
  ...
  async exportTopProductsExcel(
    ...
    res!.set({
      'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
      'Content-Disposition': 'attachment; filename=top_urunler.xlsx',
    });
    res!.send(buffer);
  }
  ...
  async exportDailySalesPdf(
    ...
    const buffer = await this.exportService.generatePdf({
      ...
      sections: [
        ...
        {
          heading: 'Ödeme Dökümü',
          content: Object.entries(data.paymentBreakdown).map(([k, v]) => [k, `₺${v}`]),
        },
      ],
    });
    res.set({
      'Content-Type': 'application/pdf',
      'Content-Disposition': `attachment; filename=satis_${date}.pdf`,
    });
    res.send(buffer);
  }
}
