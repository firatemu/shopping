...
export default function FinanceOperationsPage() {
    ...
    const handlePrintReceipt = async (opId: string) => {
        setPrinting(opId);
        try {
            const res = await api.get(`/partner-finance/operations/${opId}/receipt`);
            const wrapped = res.data as { data?: { textReceipt: string } };
            const receipt = wrapped?.data ?? (res.data as { textReceipt: string });
            const printWindow = window.open('', '_blank', 'width=600,height=800');
            if (!printWindow) return;
            printWindow.document.write(`<pre style="font-family:monospace;font-size:12px;white-space:pre-wrap">${receipt.textReceipt}</pre>`);
            printWindow.document.close();
            printWindow.print();
        } catch {
            alert('Makbuz yazdırılamadı.');
        } finally {
            setPrinting(null);
        }
    };
                                    ...
                                    <tr key={r.id} className="border-b border-border hover:bg-accent/40">
                                        ...
                                        <td className="px-4 py-2.5">
                                            <Button
                                                ...
                                                onClick={() => void handlePrintReceipt(r.id)}
                                            >
